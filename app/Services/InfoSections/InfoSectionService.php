...
namespace App\Services\InfoSections;
...
class InfoSectionService
{
    ...
    public function store(array $request)
    {
        try {
            ...
            $infoSections = $this->itemRepository->createItem($this->model, $request);

            if (isset($request['image']) && $request['image']) {
                $infoSections->addMediaFromRequest('image')->toMediaCollection('infoSections');
            }
    ...
    public function update(array $request, int $id)
    {
        try {
            ...
            $infoSections = $this->itemRepository->getItemById($this->model, $id);
            $this->itemRepository->updateItem($this->model, $id, $request);

            if (isset($request['image']) && $request['image']) {
                $infoSections->clearMediaCollection('infoSections');
                $infoSections->addMediaFromRequest('image')->toMediaCollection('infoSections');
            }
