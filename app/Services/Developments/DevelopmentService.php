...
namespace App\Services\Developments;
...
class DevelopmentService
{
    ...
    public function store(array $request)
    {
        try {
            ...
            $developments = $this->itemRepository->createItem($this->model, $request);

            if (isset($request['image']) && $request['image']) {
                $developments->addMediaFromRequest('image')->toMediaCollection('developments');
            }
    ...
    public function update(array $request, int $id)
    {
        try {
            ...
            $developments = $this->itemRepository->getItemById($this->model, $id);
            $this->itemRepository->updateItem($this->model, $id, $request);

            if (isset($request['image']) && $request['image']) {
                $developments->clearMediaCollection('developments');
                $developments->addMediaFromRequest('image')->toMediaCollection('developments');
            }
