                    ...
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <h4 class="header-title mt-0 mb-1">{{ __('attributes.infoSectionDetails') }}</h4>
                                <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100">
                                    ...
                                    <tbody>
                                        @if ($result->count())
                                            @foreach ($result as $infoSectionDetails)
                                                            ...
                                                            <div class="modal-header">
                                                                <h4 class="modal-title" id="myCenterModalLabel">
                                                                    @lang('attributes.delete') : <span
                                                                        class="text-danger">{{ $infoSectionDetails->title }}</span>
                                                                </h4>
                                                                <button type="button" class="btn-close"
                                                                    data-bs-dismiss="modal" aria-label="Close"></button>
                                                            </div>
                                                            <form id="deleteForm{{ $infoSectionDetails->id }}"
                                                                action="{{ route('admin.infoSectionDetails.delete', $infoSectionDetails->id) }}"
                                                                method="post">
                                                                @csrf
                                                                @method('delete')
                                                                ...
                                                                <div class="modal-footer">
                                                                    ...
                                                                    <button type="button" class="btn btn-danger"
                                                                        onclick="deleteData({{ $infoSectionDetails->id }})">
                                                                        @lang('attributes.delete')
                                                                    </button>
                                                                </div>
                                                            </form>
    ...
    <script>
        document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                fetch("{{ route('admin.infoSectionDetails.status', app()->getLocale()) }}", {
                    method: 'POST',
                    headers: {
