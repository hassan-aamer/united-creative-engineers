...
@section('title', __('attributes.WhyUs'))
                    ...
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <h4 class="header-title mt-0 mb-1">{{ __('attributes.WhyUs') }}</h4>
                                    ...
                                    <tbody>
                                        @if ($result->count())
                                            @foreach ($result as $WhyUs)
                                                    ...
                                                    <td>
                                                        <div class="form-check form-switch">
                                                            <input class="form-check-input" type="checkbox" name="status"
                                                                id="active-{{ $WhyUs->id }}"
                                                                @if ($WhyUs->active == 1) checked @endif
                                                                data-id="{{ $WhyUs->id }}">
                                                ...
                                                <!-- Delete Modal -->
                                                <div class="modal fade" id="delete{{ $WhyUs->id }}" tabindex="-1"
                                                            ...
                                                            <div class="modal-header">
                                                                <h4 class="modal-title" id="myCenterModalLabel">
                                                                    @lang('attributes.delete') : <span
                                                                        class="text-danger">{{ $WhyUs->title }}</span>
                                                                </h4>
                                                                <button type="button" class="btn-close"
                                                                    data-bs-dismiss="modal" aria-label="Close"></button>
                                                            </div>
                                                            <form id="deleteForm{{ $WhyUs->id }}"
                                                                action="{{ route('admin.WhyUs.delete', $WhyUs->id) }}"
                                                                method="post">
                                                                @csrf
                                                                @method('delete')
    ...
    <script>
        document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                fetch("{{ route('admin.WhyUs.status', app()->getLocale()) }}", {
                    method: 'POST',
                    headers: {
