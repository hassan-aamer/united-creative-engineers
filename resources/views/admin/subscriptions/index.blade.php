...
@section('title', __('attributes.subscriptions'))
                    ...
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <h4 class="header-title mt-0 mb-1">{{ __('attributes.subscriptions') }}</h4>
                                    ...
                                    <tbody>
                                        @if ($result->count())
                                            @foreach ($result as $subscription)
                                                        ...
                                                        @can('active subscription')
                                                            <div class="form-check form-switch">
                                                                <input class="form-check-input" type="checkbox" name="status"
                                                                    ...
                                                                    @if ($subscription->active == 1) checked @endif
                                                                    data-id="{{ $subscription->id }}">
                                                                <label class="form-check-label"
                                                                    for="active-{{ $subscription->id }}"></label>
                                                            </div>
                                                        @endcan
                                                    </td>
                                                    <td>
                                                ...
                                                <!-- Delete Modal -->
                                                <div class="modal fade" id="delete{{ $subscription->id }}" tabindex="-1"
                                                            ...
                                                            <div class="modal-header">
                                                                <h4 class="modal-title" id="myCenterModalLabel">
                                                                    @lang('attributes.delete') : <span
                                                                        class="text-danger">{{ $subscription->title }}</span>
                                                                </h4>
                                                                <button type="button" class="btn-close"
                                                                    data-bs-dismiss="modal" aria-label="Close"></button>
                                                            </div>
                                                            <form id="deleteForm{{ $subscription->id }}"
                                                                action="{{ route('admin.subscription.delete', $subscription->id) }}"
                                                                method="post">
                                                                @csrf
                                                                @method('delete')
                                                                ...
                                                                <div class="modal-footer">
                                                                    ...
                                                                    <button type="button" class="btn btn-danger"
                                                                        onclick="deleteData({{ $subscription->id }})">
                                                                        @lang('attributes.delete')
                                                                    </button>
                                                                </div>
                                                            </form>
    ...
    <script>
        document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                fetch("{{ route('admin.subscription.status', app()->getLocale()) }}", {
                    method: 'POST',
                    headers: {
