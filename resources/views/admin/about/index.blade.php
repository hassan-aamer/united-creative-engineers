                        ...
                        <div class="page-title-box">
                            @can('create about')
                                <a class="btn btn-success"
                                    href="{{ route('admin.about.create') }}">{{ __('attributes.create') }}</a>
                            @endcan
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <h4 class="header-title mt-0 mb-1">{{ __('attributes.about') }}</h4>
                                <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100">
                                    ...
                                    <tbody>
                                        @if ($result->count())
                                            @foreach ($result as $about)
                                                <tr id="row-{{ $about->id ?? '' }}">
                                                    <td>{{ $loop->iteration ?? '' }}</td>
                                                    <td>{{ $about->position ?? '' }}</td>
                                                    <td>{{ shortenText($about->title ?? '', 10) }}</td>
                                                    <td>{{ shortenText($about->description ?? '', 10) }}</td>
                                                    <td>
                                                        @can('active about')
                                                            <div class="form-check form-switch">
                                                                <input class="form-check-input" type="checkbox" name="status"
                                                                    id="active-{{ $about->id }}"
                                                                    @if ($about->active == 1) checked @endif
                                                                    data-id="{{ $about->id }}">
                                                                <label class="form-check-label"
                                                                    for="active-{{ $about->id }}"></label>
                                                            </div>
                                                        @endcan
                                                    </td>
                                                    <td>
                                                        ...
                                                        @can('delete about')
                                                            <button type="button" class="btn btn-danger btn-block btn-delete"
                                                                ...
                                                                data-bs-target="#delete{{ $about->id }}">
                                                ...
                                                <!-- Delete Modal -->
                                                <div class="modal fade" id="delete{{ $about->id }}" tabindex="-1"
                                                            ...
                                                            <div class="modal-header">
                                                                <h4 class="modal-title" id="myCenterModalLabel">
                                                                    @lang('attributes.delete') : <span
                                                                        class="text-danger">{{ $about->title }}</span>
                                                                </h4>
    ...
    <script>
        document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                fetch("{{ route('admin.about.status', app()->getLocale()) }}", {
                    method: 'POST',
                    headers: {
