...
@section('title', __('attributes.productServices'))
                    ...
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <h4 class="header-title mt-0 mb-1">{{ __('attributes.productServices') }}</h4>
                                <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100">
                                    ...
                                    <tbody>
                                        @if ($result->count())
                                            @foreach ($result as $productServices)
                                                <tr id="row-{{ $productServices->id ?? '' }}">
                                                            ...
                                                            <div class="modal-header">
                                                                <h4 class="modal-title" id="myCenterModalLabel">
                                                                    @lang('attributes.delete') : <span
                                                                        class="text-danger">{{ $productServices->title }}</span>
                                                                </h4>
                                                                <button type="button" class="btn-close"
                                                                    data-bs-dismiss="modal" aria-label="Close"></button>
                                                            </div>
                                                            <form id="deleteForm{{ $productServices->id }}"
                                                                action="{{ route('admin.productServices.delete', $productServices->id) }}"
                                                                method="post">
                                                                @csrf
                                                                @method('delete')
                                                                ...
                                                                <div class="modal-footer">
                                                                    ...
                                                                    <button type="button" class="btn btn-danger"
                                                                        onclick="deleteData({{ $productServices->id }})">
                                                                        @lang('attributes.delete')
                                                                    </button>
                                                                </div>
                                                            </form>
    ...
    <script>
        document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                fetch("{{ route('admin.productServices.status', app()->getLocale()) }}", {
                    method: 'POST',
                    headers: {
