                    ...
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <h4 class="header-title mt-0 mb-1">{{ __('attributes.features') }}</h4>
                                    ...
                                    <tbody>
                                        @if ($result->count())
                                            @foreach ($result as $features)
                                                ...
                                                <!-- Delete Modal -->
                                                <div class="modal fade" id="delete{{ $features->id }}" tabindex="-1"
                                                            ...
                                                            <div class="modal-header">
                                                                <h4 class="modal-title" id="myCenterModalLabel">
                                                                    @lang('attributes.delete') : <span
                                                                        class="text-danger">{{ $features->title }}</span>
                                                                </h4>
                                                                <button type="button" class="btn-close"
                                                                    data-bs-dismiss="modal" aria-label="Close"></button>
                                                            </div>
                                                            <form id="deleteForm{{ $features->id }}"
                                                                action="{{ route('admin.features.delete', $features->id) }}"
                                                                method="post">
                                                                @csrf
                                                                @method('delete')
                                                                ...
                                                                <div class="modal-footer">
                                                                    ...
                                                                    <button type="button" class="btn btn-danger"
                                                                        onclick="deleteData({{ $features->id }})">
                                                                        @lang('attributes.delete')
                                                                    </button>
                                                                </div>
                                                            </form>
    ...
    <script>
        document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                fetch("{{ route('admin.features.status', app()->getLocale()) }}", {
                    method: 'POST',
                    headers: {
