...
@section('title', __('attributes.faqs'))
                    ...
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <h4 class="header-title mt-0 mb-1">{{ __('attributes.faqs') }}</h4>
                                <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100">
                                    <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>@lang('attributes.position')</th>
                                            <th>@lang('attributes.question')</th>
                                            <th>@lang('attributes.answer')</th>
                                    ...
                                    <tbody>
                                        @if ($result->count())
                                            @foreach ($result as $faqs)
                                                ...
                                                <!-- Delete Modal -->
                                                <div class="modal fade" id="delete{{ $faqs->id }}" tabindex="-1"
                                                            ...
                                                            <div class="modal-header">
                                                                <h4 class="modal-title" id="myCenterModalLabel">
                                                                    @lang('attributes.delete') : <span
                                                                        class="text-danger">{{ $faqs->title }}</span>
                                                                </h4>
                                                                <button type="button" class="btn-close"
                                                                    data-bs-dismiss="modal" aria-label="Close"></button>
                                                            </div>
                                                            <form id="deleteForm{{ $faqs->id }}"
                                                                action="{{ route('admin.faqs.delete', $faqs->id) }}"
                                                                method="post">
                                                                @csrf
                                                                @method('delete')
    ...
    <script>
        document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                fetch("{{ route('admin.faqs.status', app()->getLocale()) }}", {
                    method: 'POST',
                    headers: {
